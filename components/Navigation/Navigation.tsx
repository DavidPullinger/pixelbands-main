...
export default function Navigation() {
  ...
  return (
    <div className="relative flex items-center justify-between z-10 pb-8 lg:pb-12">
      ...
      {menuOpen ? <XIcon onClick={() => handleMenuToggle(false)} fill="white" className="absolute w-8 right-0 top-0 z-10 cursor-pointer md:hidden" /> : <MenuIcon onClick={() => handleMenuToggle(true)} fill="white" className="absolute w-8 right-0 top-0 z-10 cursor-pointer md:hidden" />}
      <ul
        data-visible="false"
        id="menu"
      ...
      >
        ...
        <div className="flex gap-4 mt-auto items-center md:ml-6">
          <IconLink url="https://twitter.com/PixelBandsNFT" src={Twitter} width={24} height={24} />
          <IconLink url="https://discord.gg/pixelbands" src={Discord} width={24} height={24} />
          <IconLink url="https://magiceden.io/creators/pixelbands" src={MagicEden} width={24} height={24} />
        </div>
      </ul>
    </div>
  );
